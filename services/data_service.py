...
class DataService:
    ...
    def add(self, artist: str, name: str, year: int, gender:str):
        self.data.append(Disk(artist, name, year, gender))

    def update(self, artist: str, name: str, year: int, gender:str):
    ...
    def get_all_by_year(self, gender: str):
    ...
    def load(self):
        with open('./data/data.json') as file:
            temp = json.load(file)
            for x in temp:
                self.data.append(Disk(x['artist'], x['name'], int(x['year']), x['gender']))
